...
class ComplEx(Model):
    ...
    def get_batch_embeddings(self, data):
        h_re = self.ent_re_embeddings(data['batch_h'])
        h_im = self.ent_im_embeddings(data['batch_h'])
        t_re = self.ent_re_embeddings(data['batch_t'])
        t_im = self.ent_im_embeddings(data['batch_t'])
        r_re = self.rel_re_embeddings(data['batch_r'])
        r_im = self.rel_im_embeddings(data['batch_r'])
        return h_re, h_im, t_re, t_im, r_re, r_im